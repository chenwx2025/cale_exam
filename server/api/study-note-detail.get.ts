...
export default defineEventHandler(async (event) => {
  console.log('[FLAT NOTE DETAIL] ========== 请求到达 ==========')
  ...
  console.log('[FLAT NOTE DETAIL] 用户:', user.userId, 'groupId:', groupId, 'noteId:', noteId)
  ...
  try {
    console.log('[FLAT NOTE DETAIL] 开始验证用户权限')
    ...
    if (!membership) {
      console.log('[FLAT NOTE DETAIL] 用户不是小组成员')
      throw createError({
        statusCode: 403,
    ...
    console.log('[FLAT NOTE DETAIL] 用户是小组成员，开始获取笔记详情')
    ...
    if (!note) {
      console.log('[FLAT NOTE DETAIL] 笔记不存在')
      throw createError({
        statusCode: 404,
        message: '笔记不存在'
      })
    ...
    if (note.groupId !== groupId) {
      console.log('[FLAT NOTE DETAIL] 笔记不属于此小组')
      throw createError({
        statusCode: 403,
        message: '无权访问该笔记'
      })
    ...
    console.log('[FLAT NOTE DETAIL] 笔记详情获取成功')

    // 增加浏览量（异步，不等待）
    prisma.studyNote.update({
      where: { id: noteId },
      data: { viewCount: { increment: 1 } }
    }).catch(err => console.error('[FLAT NOTE DETAIL] 更新浏览量失败:', err))
    ...
    console.log('[FLAT NOTE DETAIL] 准备返回响应...')
    ...
    console.error('[FLAT NOTE DETAIL] 获取笔记详情失败:', error)
    throw createError({
      statusCode: 500,
      message: '获取笔记详情失败'
    })
